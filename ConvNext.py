...
class ConvNextLayer(nn.Module):
    ...
    def forward(self, x):
        for idx, block in enumerate(self.blocks):
            x = x + stochastic_depth(block(x),self.drop_rates[idx],mode="batch",training=self.training)
        return x

class ConvNext(nn.Module):
    def __init__(self,num_channels=3,num_classes=10,patch_size=4,layer_dims=[96, 192, 384, 768],depths=[3, 3, 9, 3],drop_rate=0.0):
        super().__init__()
